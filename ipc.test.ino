...
// --- THE BULLETPROOF HARDCODED MATRICES ---
const byte RED_ROW[3]   = {6, 9, 3};
const byte GREEN_ROW[3] = {5, 8, 2};
const byte BLUE_ROW[3]  = {4, 7, 10};

// Columns ordered logically: Left (13), Middle (11), Right (12)
...
void lightOne(byte r, byte c, byte color) {
  ...
  if (color == 1) digitalWrite(BLUE_ROW[r], HIGH);
  if (color == 2) digitalWrite(RED_ROW[r], HIGH);
  if (color == 3) digitalWrite(GREEN_ROW[r], HIGH);
...
void checkWin() {
  ...
  if (isFull && !gameOver) gameOver = true;
}
...
int readRawKey() {
  for (byte r = 0; r < 3; r++) {
    for (byte x = 0; x < 3; x++) pinMode(KP_ROW[x], INPUT);
...
void processInput() {
  ...
  if (raw != -1 && raw != lastRaw) {
    ...
    byte physCol = 0;
    if (rawCol == 1) physCol = 0;      // Left
    else if (rawCol == 0) physCol = 1; // Middle
    else if (rawCol == 2) physCol = 2; // Right
  ...
  if (raw == -1) lastRaw = -1; // Reset lock when released
}
